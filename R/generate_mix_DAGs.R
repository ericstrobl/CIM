generate_mix_DAGs <- function(nIndep, p, en, waves){
  ...
  mDAGs$waves= list(w1=1:length(mDAGs$waves$w1), w2=(length(mDAGs$waves$w1)+1):(length(mDAGs$waves$w1)+length(mDAGs$waves$w2)), w3=(length(mDAGs$waves$w1)+length(mDAGs$waves$w2)+1):(length(mDAGs$waves$w1)+length(mDAGs$waves$w2)+length(mDAGs$waves$w3)) ) # reorganize waves according to generated latent and selection variables
